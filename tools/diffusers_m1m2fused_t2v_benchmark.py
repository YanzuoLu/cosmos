...
def parse_args() -> argparse.Namespace:
    ...
    fuse_gate_up = parser.add_mutually_exclusive_group()
    fuse_gate_up.add_argument(
        "--fuse-gate-up",
        dest="fuse_gate_up",
        action="store_true",
        help="Fuse mlp_moe_gen gate/up projections into one horizontal GEMM.",
    )
    fuse_gate_up.add_argument(
        "--no-fuse-gate-up",
        dest="fuse_gate_up",
        action="store_false",
        help="Disable M3 gate/up fusion for A/B checks.",
    )
    parser.set_defaults(fuse_gate_up=False)
...
def m1_fp8_filter_fn(module: Any, fqn: str) -> bool:
    ...
    if fqn.endswith((".self_attn.add_q_proj", ".self_attn.add_k_proj", ".self_attn.add_v_proj", ".self_attn.to_add_out")):
        return True
    if ".mlp_moe_gen." in fqn and fqn.rsplit(".", 1)[-1] in {"gate_proj", "up_proj", "fused_gate_up_proj", "down_proj"}:
        return True
    return False
...
def m3_torchao_per_row_weight_scale(weight: Any) -> Any:
    import torch
    from torchao.quantization import PerRow
    from torchao.quantization.quant_primitives import _choose_scale_float8
    from torchao.quantization.utils import get_block_size

    block_size = list(get_block_size(tuple(weight.shape), PerRow()))
    return _choose_scale_float8(
        weight.detach(),
...
def m3_max_abs_diff(lhs: Any, rhs: Any) -> float:
    import torch

    return float((lhs.to(dtype=torch.float32) - rhs.to(dtype=torch.float32)).abs().max().item())


_M3_FUSED_GATE_UP_MLP_CLASS = None


def m3_get_fused_gate_up_mlp_class() -> Any:
    global _M3_FUSED_GATE_UP_MLP_CLASS
    if _M3_FUSED_GATE_UP_MLP_CLASS is not None:
        return _M3_FUSED_GATE_UP_MLP_CLASS

    import torch.nn as nn

    class M3FusedGateUpSwiGLU(nn.Module):
        def __init__(
            self,
            fused_gate_up_proj: Any,
            down_proj: Any,
            act_fn: Any,
            gate_scale_cpu: Any,
            up_scale_cpu: Any,
            ...
            super().__init__()
            self.fused_gate_up_proj = fused_gate_up_proj
            self.down_proj = down_proj
            self.act_fn = act_fn
            self._m3_separate_gate_row_scale_cpu = gate_scale_cpu
            self._m3_separate_up_row_scale_cpu = up_scale_cpu

        def forward(self, x):
            gate, up = self.fused_gate_up_proj(x).chunk(2, dim=-1)
            return self.down_proj(self.act_fn(gate) * up)

    _M3_FUSED_GATE_UP_MLP_CLASS = M3FusedGateUpSwiGLU
    return _M3_FUSED_GATE_UP_MLP_CLASS


def m3_build_fused_gate_up_mlp(original_mlp: Any, fused_gate_up_proj: Any, gate_scale_cpu: Any, up_scale_cpu: Any) -> Any:
    fused_mlp = m3_get_fused_gate_up_mlp_class()(
        fused_gate_up_proj,
    ...
    fused_mlp.train(original_mlp.training)
    return fused_mlp


def apply_m3_gate_up_fusion(transformer: Any) -> dict[str, Any]:
    import torch
    import torch.nn as nn

    if getattr(transformer, "_m3_gate_up_fused", False):
        raise RuntimeError("M3 gate/up fusion was already applied to this transformer instance")

    layers = getattr(transformer, "layers", [])
    smoothquant_folded = bool(getattr(transformer, "_m1_smoothquant_folded", False))
    smoothquant_consumer_scales = getattr(transformer, "_m1_smoothquant_consumer_scales", None)
    if smoothquant_folded and smoothquant_consumer_scales is None:
        raise RuntimeError("M1 SmoothQuant was folded without recorded consumer scales")

    max_smoothquant_scale_diff = 0.0 if smoothquant_folded else None
    max_prequant_gate_row_scale_diff = 0.0
    max_prequant_up_row_scale_diff = 0.0
    smoothquant_bit_identical = True
    prequant_gate_bit_identical = True
    prequant_up_bit_identical = True
    ...
    with torch.no_grad():
        for layer_index, layer in enumerate(layers):
            ...
            if not isinstance(gate_proj, nn.Linear) or not isinstance(up_proj, nn.Linear):
                raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen does not expose Linear gate_proj/up_proj")
            if not isinstance(down_proj, nn.Linear):
                raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen.down_proj is not a Linear")
            if gate_proj.bias is not None or up_proj.bias is not None:
                raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen gate/up bias is unsupported by M3 fusion")
            if gate_proj.in_features != up_proj.in_features or gate_proj.out_features != up_proj.out_features:
                raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen gate/up shapes do not match")

            smoothquant_scale_diff = None
            if smoothquant_folded:
                gate_key = f"layers.{layer_index}.mlp_moe_gen.gate_proj"
                up_key = f"layers.{layer_index}.mlp_moe_gen.up_proj"
                gate_smoothquant_scale = smoothquant_consumer_scales.get(gate_key)
                up_smoothquant_scale = smoothquant_consumer_scales.get(up_key)
                if gate_smoothquant_scale is None or up_smoothquant_scale is None:
                    raise RuntimeError(f"missing recorded SmoothQuant scales for {gate_key} / {up_key}")
                smoothquant_scale_diff = m3_max_abs_diff(gate_smoothquant_scale, up_smoothquant_scale)
                max_smoothquant_scale_diff = max(max_smoothquant_scale_diff, smoothquant_scale_diff)
                layer_smoothquant_exact = torch.equal(gate_smoothquant_scale, up_smoothquant_scale)
                smoothquant_bit_identical = smoothquant_bit_identical and layer_smoothquant_exact
                if smoothquant_scale_diff != 0.0 or not layer_smoothquant_exact:
                    raise RuntimeError(
                        f"SmoothQuant input-column scale mismatch in layer {layer_index}: {smoothquant_scale_diff}"
                    )

            gate_scale_cpu = m3_torchao_per_row_weight_scale(gate_proj.weight).detach().to(device="cpu", dtype=torch.float32)
            up_scale_cpu = m3_torchao_per_row_weight_scale(up_proj.weight).detach().to(device="cpu", dtype=torch.float32)

            fused_gate_up_proj = nn.Linear(
            ...
            fused_gate_up_proj.weight.copy_(torch.cat((gate_proj.weight.detach(), up_proj.weight.detach()), dim=0))
            fused_gate_up_proj.weight.requires_grad_(gate_proj.weight.requires_grad or up_proj.weight.requires_grad)

            fused_scale_cpu = m3_torchao_per_row_weight_scale(fused_gate_up_proj.weight).detach().to(
                device="cpu", dtype=torch.float32
            )
            intermediate_size = gate_proj.out_features
            gate_row_scale_diff = m3_max_abs_diff(fused_scale_cpu[:intermediate_size], gate_scale_cpu)
            up_row_scale_diff = m3_max_abs_diff(fused_scale_cpu[intermediate_size:], up_scale_cpu)
            max_prequant_gate_row_scale_diff = max(max_prequant_gate_row_scale_diff, gate_row_scale_diff)
            max_prequant_up_row_scale_diff = max(max_prequant_up_row_scale_diff, up_row_scale_diff)
            gate_exact = torch.equal(fused_scale_cpu[:intermediate_size], gate_scale_cpu)
            up_exact = torch.equal(fused_scale_cpu[intermediate_size:], up_scale_cpu)
            prequant_gate_bit_identical = prequant_gate_bit_identical and gate_exact
            prequant_up_bit_identical = prequant_up_bit_identical and up_exact
            if gate_row_scale_diff != 0.0 or up_row_scale_diff != 0.0 or not gate_exact or not up_exact:
                raise RuntimeError(
                    "TorchAO per-row scale mismatch before FP8 apply in "
                    f"layer {layer_index}: gate={gate_row_scale_diff}, up={up_row_scale_diff}"
                )

            setattr(
                layer,
                "mlp_moe_gen",
                m3_build_fused_gate_up_mlp(mlp, fused_gate_up_proj, gate_scale_cpu, up_scale_cpu),
            )
            fused_count += 1
            if len(sample) < 4:
                sample.append(
                    {
                        ...
                        "fused_out_features": fused_gate_up_proj.out_features,
                        "weight_dtype": str(fused_gate_up_proj.weight.dtype),
                        "weight_device": str(fused_gate_up_proj.weight.device),
                        "smoothquant_scale_max_abs_diff": smoothquant_scale_diff,
                        "prequant_gate_row_scale_max_abs_diff": gate_row_scale_diff,
                        "prequant_up_row_scale_max_abs_diff": up_row_scale_diff,
                    }
                )

    if fused_count != len(layers):
        raise RuntimeError(f"M3 gate/up fusion replaced {fused_count} MLPs for {len(layers)} layers")

    record = {
        "event": "m3_gate_up_fusion_applied",
        "enabled": True,
        "layer_count": len(layers),
        "fused_count": fused_count,
        "smoothquant_folded": smoothquant_folded,
        "smoothquant_scale_max_abs_diff": max_smoothquant_scale_diff,
        "smoothquant_scale_bit_identical": smoothquant_bit_identical if smoothquant_folded else None,
        "prequant_gate_row_scale_max_abs_diff": max_prequant_gate_row_scale_diff,
        "prequant_up_row_scale_max_abs_diff": max_prequant_up_row_scale_diff,
        "prequant_gate_row_scale_bit_identical": prequant_gate_bit_identical,
        "prequant_up_row_scale_bit_identical": prequant_up_bit_identical,
        "sample": sample,
    }
    setattr(transformer, "_m3_gate_up_fused", True)
    setattr(transformer, "_m3_gate_up_fusion_record", record)
    print(json.dumps(record, sort_keys=True), flush=True)
    return record


def verify_m3_gate_up_fp8_scales(transformer: Any) -> dict[str, Any]:
    import torch

    max_gate_scale_diff = 0.0
    max_up_scale_diff = 0.0
    gate_bit_identical = True
    up_bit_identical = True
    layers_checked = 0
    sample: list[dict[str, Any]] = []

    for layer_index, layer in enumerate(getattr(transformer, "layers", [])):
        mlp = getattr(layer, "mlp_moe_gen", None)
        fused_gate_up_proj = getattr(mlp, "fused_gate_up_proj", None)
        if fused_gate_up_proj is None:
            raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen has no fused_gate_up_proj")
        weight = getattr(fused_gate_up_proj, "weight", None)
        scale = getattr(weight, "scale", None)
        if scale is None:
            raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen.fused_gate_up_proj weight has no FP8 scale")
        gate_ref = getattr(mlp, "_m3_separate_gate_row_scale_cpu", None)
        up_ref = getattr(mlp, "_m3_separate_up_row_scale_cpu", None)
        if gate_ref is None or up_ref is None:
            raise RuntimeError(f"layers.{layer_index}.mlp_moe_gen missing M3 separate row scale references")

        scale_cpu = scale.detach().to(device="cpu", dtype=torch.float32)
        intermediate_size = gate_ref.shape[0]
        gate_actual = scale_cpu[:intermediate_size]
        up_actual = scale_cpu[intermediate_size:]
        gate_diff = m3_max_abs_diff(gate_actual, gate_ref)
        up_diff = m3_max_abs_diff(up_actual, up_ref)
        max_gate_scale_diff = max(max_gate_scale_diff, gate_diff)
        max_up_scale_diff = max(max_up_scale_diff, up_diff)
        gate_exact = torch.equal(gate_actual, gate_ref)
        up_exact = torch.equal(up_actual, up_ref)
        gate_bit_identical = gate_bit_identical and gate_exact
        up_bit_identical = up_bit_identical and up_exact
        if gate_diff != 0.0 or up_diff != 0.0 or not gate_exact or not up_exact:
            raise RuntimeError(f"M3 FP8 row scale mismatch in layer {layer_index}: gate={gate_diff}, up={up_diff}")
        layers_checked += 1
        if len(sample) < 4:
            sample.append(
                {
                    "layer": layer_index,
                    "gate_scale_max_abs_diff": gate_diff,
                    "up_scale_max_abs_diff": up_diff,
                    "scale_shape": list(scale_cpu.shape),
                }
            )

    record = {
        "event": "m3_gate_up_fp8_scale_check",
        "layers_checked": layers_checked,
        "gate_scale_max_abs_diff": max_gate_scale_diff,
        "up_scale_max_abs_diff": max_up_scale_diff,
        "gate_scale_bit_identical": gate_bit_identical,
        "up_scale_bit_identical": up_bit_identical,
        "sample": sample,
    }
    print(json.dumps(record, sort_keys=True), flush=True)
    return record
...
def apply_m1_smoothquant(
    ...
    *,
    ...
    with torch.no_grad():
        for group_key in expected_group_keys:
            ...
            for consumer_key, consumer in zip(consumer_keys, consumers):
                ...
                consumer_scales[consumer_key] = scale.clone()
    ...
    setattr(transformer, "_m1_smoothquant_consumer_scales", consumer_scales)
    record = {
...
def apply_m1_fp8(transformer: Any) -> dict[str, Any]:
    ...
    m3_gate_up_fused = bool(getattr(transformer, "_m3_gate_up_fused", False))
    expected_hit_count = 216 if m3_gate_up_fused else 252
    hits = [
        name
    ...
    record: dict[str, Any] = {
        ...
        "m3_gate_up_fused": m3_gate_up_fused,
        "counts": counts,
        "sample": hits[:14],
        "config": repr(cfg),
    }
    print(json.dumps(record, sort_keys=True), flush=True)
    if len(hits) != expected_hit_count:
        print(
            json.dumps(
                {
        ...
        raise RuntimeError(f"M1 FP8 filter matched {len(hits)} modules, expected {expected_hit_count}")

    quantize_(transformer, cfg, filter_fn=m1_fp8_filter_fn)
    if m3_gate_up_fused:
        record["m3_gate_up_fp8_scale_check"] = verify_m3_gate_up_fp8_scales(transformer)
...
_M1M2_SPARGE_FP8_OP = None
_M1M2_ORIGINAL_SPARGE_BACKEND = None
_M1M2_ORIGINAL_SPARGE_CONSTRAINTS = None


def get_m1m2_sparge_fp8_op():
    global _M1M2_SPARGE_FP8_OP
    if _M1M2_SPARGE_FP8_OP is not None:
        return _M1M2_SPARGE_FP8_OP

    import torch

    @torch.library.custom_op(
        "m1m2::sparge_fp8_dense_sm90",
        mutates_args=(),
        device_types="cuda",
    ...
    _M1M2_SPARGE_FP8_OP = sparge_fp8_dense_sm90
    return sparge_fp8_dense_sm90


def install_m1m2_sparge_opaque_backend() -> dict[str, Any]:
    ...
    def _m1m2_sparge_attention(
        ...
        if attn_mask is not None:
            raise ValueError("`attn_mask` is not supported for m1m2 sparge attention")
        if dropout_p != 0.0:
            raise ValueError("`dropout_p` is not supported for m1m2 sparge attention")
        if return_lse:
            raise ValueError("`return_lse` is not supported for m1m2 sparge attention")
        if _parallel_config is not None:
            raise ValueError("context-parallel is not supported for m1m2 sparge attention")
    ...
    record = {
        ...
        "custom_op": "m1m2::sparge_fp8_dense_sm90",
...
def load_taylorseer_pipeline(args: argparse.Namespace, dtype: Any, config: dict[str, Any]):
    ...
    if args.attention_backend and args.attention_backend != "native":
        ...
        if args.attention_backend == "sparge":
            ...
            attention_backend_record = install_m1m2_sparge_opaque_backend()
    ...
    if args.fuse_gate_up:
        m3_gate_up_fusion = apply_m3_gate_up_fusion(pipe.transformer)
    if args.m1_fp8:
        ...
        json.dumps(
            {
                ...
                "m3_gate_up_fusion": m3_gate_up_fusion,
