...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="sparge",
        help="diffusers attention backend for the gen full-attention; 'sparge' = M2 Phase-A SpargeAttn quant-only, 'native' = baseline SDPA",
    )
...
def load_taylorseer_pipeline(args: argparse.Namespace, dtype: Any, config: dict[str, Any]):
    ...
    if args.attention_backend and args.attention_backend != "native":
        ...
        print(f"[M2] attention backend set to: {args.attention_backend}")
    cuda_sync(args.device)
    seconds = time.perf_counter() - t0
    print(
        json.dumps(
