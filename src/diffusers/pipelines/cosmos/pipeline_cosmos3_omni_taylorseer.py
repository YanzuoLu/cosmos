...
from ...models.transformers.transformer_cosmos3_taylorseer import (
...
@dataclass
class Cosmos3OmniTaylorSeerPipelineOutput(BaseOutput):
    """Output dataclass for :class:`Cosmos3OmniTaylorSeerPipeline`.
...
class CosmosActionCondition:
    ...
    Pass this to [`Cosmos3OmniTaylorSeerPipeline.__call__`] via the `action` argument instead of the top-level `image` / `height`
...
class Cosmos3OmniTaylorSeerPipeline(DiffusionPipeline):
    ...
    def __init__(
        self,
        transformer: Cosmos3OmniTaylorSeerTransformer,
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        if kwargs.get("transformer") is None:
            ...
            transformer_kwargs = {key: kwargs[key] for key in transformer_keys if key in kwargs}
            kwargs["transformer"] = Cosmos3OmniTaylorSeerTransformer.from_pretrained(
    ...
    def enable_taylorseer(self, **kwargs: Any) -> None:
        self.transformer.enable_taylorseer(**kwargs)

    def disable_taylorseer(self) -> None:
        self.transformer.disable_taylorseer()

    def reset_taylorseer_cache(self, *, clear_stats: bool = True) -> None:
        self.transformer.reset_taylorseer_cache(clear_stats=clear_stats)

    def get_taylorseer_stats(self) -> dict[str, Any]:
        return self.transformer.get_taylorseer_stats()
    ...
    def __call__(
        ...
        enable_taylorseer: bool | None = None,
        taylorseer_interval: int = 5,
        taylorseer_fresh_threshold: int | None = None,
        taylorseer_force_scheduler: bool = False,
        taylorseer_max_order: int = 1,
        taylorseer_first_enhance: int = 1,
        taylorseer_last_enhance: int = 1,
        taylorseer_force_final_full: bool = True,
        taylorseer_branches: Literal["both", "cond", "uncond"] = "both",
        taylorseer_delta_change_threshold: float | None = None,
        taylorseer_prediction_target: Literal["layer_delta", "attention_delta", "mlp_delta", "gen_component_delta", "und_cache"] = "layer_delta",
        taylorseer_layer_indices: tuple[int, ...] | None = None,
        taylorseer_cache_und: bool = True,
        taylorseer_stagger_layers: bool = False,
        taylorseer_slope_scale: float = 1.0,
        taylorseer_cache_max_gib: float | None = 64.0,
    ) -> Cosmos3OmniTaylorSeerPipelineOutput:
        r"""
        ...
        Args:
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                When `True`, returns a [`Cosmos3OmniTaylorSeerPipelineOutput`]; otherwise a plain tuple `(video, sound)`.
            ...
            callback_on_step_end_tensor_inputs (`List[str]`, *optional*, defaults to `["latents"]`):
                ...
                [`~Cosmos3OmniTaylorSeerPipeline._callback_tensor_inputs`].
            ...
            enable_taylorseer (`bool`, *optional*):
                `None` keeps the transformer's persistent TaylorSeer setting. `True` enables TaylorSeer for this call
                with the supplied TaylorSeer options, and `False` disables it for this call. Temporary overrides restore
                the previous transformer configuration before returning.
            taylorseer_interval (`int`, *optional*, defaults to `5`):
                Denoising-step interval between full selected decoder-layer evaluations when TaylorSeer is enabled for
                this call.
            taylorseer_fresh_threshold (`int`, *optional*):
                Upstream-style full/Taylor cadence threshold. When set, this controls the refresh interval used by the
                transformer's local `cal_type()` compatibility path instead of the plain interval cadence.
            taylorseer_force_scheduler (`bool`, *optional*, defaults to `False`):
                Whether to store the per-full-step cadence threshold through the local analogue of upstream
                `force_scheduler()`.
            taylorseer_max_order (`int`, *optional*, defaults to `1`):
                Taylor predictor order passed to the transformer (`0` or `1`).
            taylorseer_first_enhance (`int`, *optional*, defaults to `1`):
                Number of initial denoising steps forced to full selected-layer evaluation.
            taylorseer_last_enhance (`int`, *optional*, defaults to `1`):
                Number of final denoising steps forced to full selected-layer evaluation when final full forcing is enabled.
            taylorseer_force_final_full (`bool`, *optional*, defaults to `True`):
                Whether the final denoising step must evaluate selected decoder layers exactly.
            taylorseer_branches (`str`, *optional*, defaults to `"both"`):
                Which classifier-free-guidance branch uses TaylorSeer approximation: both branches, conditional only,
                or unconditional only. The other branch remains exact.
            taylorseer_delta_change_threshold (`float`, *optional*):
                Optional RMS-normalized GEN-delta change guard. When set, a selected layer must show stable
                consecutive full-step deltas before a predicted step is allowed.
            taylorseer_prediction_target (`str`, *optional*, defaults to `"layer_delta"`):
                Approximation target for selected decoder layers. `"layer_delta"` skips the full layer, `"attention_delta"`
                reuses only the attention residual and still evaluates the GEN MLP, `"mlp_delta"` evaluates attention
                plus the UND MLP exactly while reusing only the GEN MLP residual, `"gen_component_delta"`
                predicts the GEN attention and GEN MLP residuals separately, and `"und_cache"` exactly caches the
                static understanding pathway while evaluating the generation pathway exactly.
            taylorseer_layer_indices (`tuple[int, ...]`, *optional*):
                Selected decoder-layer indices. `None` lets the transformer use every decoder layer.
            taylorseer_cache_und (`bool`, *optional*, defaults to `True`):
                Whether predicted `"layer_delta"`, `"attention_delta"`, and `"gen_component_delta"` steps reuse cached UND states. Set to `False`
                to recompute the current UND pathway exactly on predicted steps.
            taylorseer_stagger_layers (`bool`, *optional*, defaults to `False`):
                Whether selected layers use staggered full-evaluation phases instead of refreshing every selected layer
                on the same denoising steps.
            taylorseer_slope_scale (`float`, *optional*, defaults to `1.0`):
                Scale applied to the first-order Taylor slope term. Values below `1.0` damp the extrapolation while
                preserving the cached full-step delta.
            taylorseer_cache_max_gib (`float`, *optional*, defaults to `64.0`):
                Cache preflight cap in GiB. `None` disables the cap.

        Returns:
            [`Cosmos3OmniTaylorSeerPipelineOutput`] or `tuple`:
                If `return_dict=True`, a [`Cosmos3OmniTaylorSeerPipelineOutput`] with `video` (typed per `output_type`) and
        ...
        previous_taylorseer_config = copy.deepcopy(self.transformer._taylorseer_config)
        restore_taylorseer_config = enable_taylorseer is not None
        taylorseer_run_started = False
        try:
            if enable_taylorseer is True:
                self.transformer.enable_taylorseer(
                    interval=taylorseer_interval,
                    fresh_threshold=taylorseer_fresh_threshold,
                    force_scheduler=taylorseer_force_scheduler,
                    max_order=taylorseer_max_order,
                    first_enhance=taylorseer_first_enhance,
                    last_enhance=taylorseer_last_enhance,
                    force_final_full=taylorseer_force_final_full,
                    branches=taylorseer_branches,
                    delta_change_threshold=taylorseer_delta_change_threshold,
                    prediction_target=taylorseer_prediction_target,
                    layer_indices=taylorseer_layer_indices,
                    cache_und=taylorseer_cache_und,
                    stagger_layers=taylorseer_stagger_layers,
                    slope_scale=taylorseer_slope_scale,
                    cache_max_gib=taylorseer_cache_max_gib,
                )
            elif enable_taylorseer is False:
                self.transformer.disable_taylorseer()
            taylorseer_enabled_for_call = self.transformer.is_taylorseer_enabled()
            if taylorseer_enabled_for_call and (enable_sound or action is not None or image is not None):
                raise ValueError(
                    "TaylorSeer acceleration is currently validated only for Cosmos3 text-to-video runs with "
                    "image/sound/action conditioning disabled"
                )
            ...
            if taylorseer_enabled_for_call:
                self.transformer.begin_taylorseer_run(
                    num_steps=len(timesteps),
                    do_classifier_free_guidance=self.do_classifier_free_guidance,
                )
                taylorseer_run_started = True
            try:
                ...
                with self.progress_bar(total=num_inference_steps) as progress_bar:
                    for i, t in enumerate(timesteps):
                        ...
                        # --- Conditional pass ---
                        if taylorseer_enabled_for_call:
                            self.transformer.set_taylorseer_context(branch="cond", step_index=i, timestep=timestep)
                        try:
                            ...
                            if taylorseer_enabled_for_call:
                                self.transformer.clear_taylorseer_context()
                        ...
                        if self.do_classifier_free_guidance:
                            if taylorseer_enabled_for_call:
                                self.transformer.set_taylorseer_context(branch="uncond", step_index=i, timestep=timestep)
                            try:
                                preds_vision, preds_sound, preds_action = self.transformer(
                ...
                self._current_timestep = None
                if taylorseer_run_started:
                    self.transformer.finish_taylorseer_run()
                    taylorseer_run_started = False
                ...
                if not return_dict:
                    ...
                    return (video, sound)
                return Cosmos3OmniTaylorSeerPipelineOutput(video=video, sound=sound, action=action_output)
            finally:
                self.transformer.clear_taylorseer_context()
                if taylorseer_run_started:
                    self.transformer.finish_taylorseer_run()
        finally:
            if restore_taylorseer_config:
                self.transformer._taylorseer_config = previous_taylorseer_config
